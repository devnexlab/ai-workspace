...
def _run_screening_job(sid, payload):
    ...
    try:
        def progress(done, total):
            ...
            _update_screening(sid, status='running', message=f'扫描中 {done}/{total}')
        ...
        result = run_screen(
            ...
            max_stocks=int(
                payload.get('max_stocks')
                if payload.get('max_stocks') is not None
                else (get_setting('stock', 'max_stocks', '300') or 300)
            ),
...
def update_strategy(id):
    ...
    conn = _db()
    fields, params = [], []
    for k in ['name', 'description', 'strategy_type', 'rules_json', 'score',
              'hit_rate', 'total_trades', 'winning_trades', 'status']:
        if k in data:
            fields.append(f'{k}=%s')
            val = data[k]
...
{{
  ...
  "win_rate_trend": "历史胜率变化趋势"
    ...
    try:
        ...
        return jsonify({'review': result, 'result': result, 'message': 'AI复盘完成'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def stock_to_knowledge():
    ...
    cur = conn.execute(
        ...
        (title, content, data.get('category') or '股票交易', tags, code)
    )
    new_id = cur.lastrowid
    conn.commit()
    conn.close()
